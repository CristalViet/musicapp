<!-- Side widgets-->
<div class="col-lg-4">
                  
    <!-- Categories widget-->
    <div class=" mb-4">
        <div class="card-header text-primary fs-3">Chủ đề HÓT</div>
        <div class="card-body">
            <div class="row">
                <div class="col-sm-12">
                    <ul class="list-unstyled mb-0">
                        <li class="mb-1"><a title="RAP VIỆT" href="#!"><img src="https://avatar-ex-swe.nixcdn.com/topic/thumb/2020/06/22/9/7/b/e/1592812587983_org.jpg" alt=""></a>
                            
                        </li>
                        <li  class="mb-1"><a title="RAP VIỆT" href="#!"><img src="https://avatar-ex-swe.nixcdn.com/topic/thumb/2020/06/22/9/7/b/e/1592812587983_org.jpg" alt=""></a>
                            
                        </li>
                        
                    </ul>
                </div>
                
            </div>
        </div>
    </div>
    <!-- Side widget-->
    <div class=" mb-4">
        
        <div class="card-header  fs-4 text-primary">Bảng Xếp Hạng</div>
       <ul class="list-unstyled">
            {{-- record bài hát --}}
            <li class="row mb-2 ">
                <div class="col-4 ">
                    <a href="" class="unlink thumbnail-music">
                        <img class="img-thumbnail " src="data:image/webp;base64," alt="">
                    <span class="number special-1">
                        1
                    </span>
                    </a>
                    
                </div>
                <div class="col-7 row">
                    <div>
                        Tên bài hát
                    </div>
                    <div>
                        Nghệ sĩ
                    </div>
                </div>
            </li>
            {{-- record bài hát --}}
            <li class="row  mb-2">
                <div class="col-1 ">
                   
                    <span class="number special-2">
                        2
                    </span>
                   
                    
                </div>
                <div class="col-10 row">
                    <div>
                        Tên bài hát
                    </div>
                    <div>
                        Nghệ sĩ
                    </div>
                </div>
            </li>
            {{-- record bài hát --}}
            <li class="row mb-2 ">
                <div class="col-1 ">
                   
                    <span class="number special-3">
                        2
                    </span>
                   
                    
                </div>
                <div class="col-10 row">
                    <div>
                        Tên bài hát
                    </div>
                    <div>
                        Nghệ sĩ
                    </div>
                </div>
            </li>

            {{-- record bài hát --}}
            <li class="row mb-2 ">
                <div class="col-1 ">
                   
                    <span class="number ">
                        3
                    </span>
                   
                    
                </div>
                <div class="col-10 row">
                    <div>
                        Tên bài hát
                    </div>
                    <div>
                        Nghệ sĩ
                    </div>
                </div>
            </li>
       </ul>
    </div>
</div>